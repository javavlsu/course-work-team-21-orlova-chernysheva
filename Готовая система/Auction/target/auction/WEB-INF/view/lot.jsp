    ...
    <style>
        * {
        ...
        .image-container img {
          max-width: 100%;
          max-height: 100%;
          object-fit: contain; /* сохраняет пропорции, показывает всё изображение */
          border-radius: 8px;
...
<c:choose>
    <c:when test="${lot.user.id != sessionScope.userId}">
        <div class="bid-form">
            ...
            <form action="${pageContext.request.contextPath}/placeBid" method="post">
                ...
                <input
                    ...
                    min="${lot.currentPrice != null ? lot.currentPrice + 1 : lot.startPrice}"
                    required
                    step="1"
                >
    ...
    <c:otherwise>
        <div class="bid-form">
            <h3>Сделать ставку</h3>
            <p class="error-message">Вы не можете делать ставки на собственные лоты.</p>
        </div>
    </c:otherwise>
</c:choose>
</div>
...
document.querySelector('form')?.addEventListener('submit', function(e) {
    ...
    const currentPrice = ${lot.currentPrice != null ? lot.currentPrice : lot.startPrice};

    if (parseFloat(amountInput.value) <= currentPrice) {
